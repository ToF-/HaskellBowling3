...
type Throw = Int
type Score = Int
type Bonus = (Bool,Bool)

score :: [Throw] -> Score
score = _Score . foldl add initial 
    where
    initial :: GameState 
    initial = GameState { _Score = 0, _Bonus = (False,False), _Last = Nothing }
    ...
    add (GameState sc b l) t = GameState { _Score = newScore sc b t, 
        ...
        newScore sc (False,_) t = sc + t 
        newScore sc (True,_)  t = sc + t + t

        newBonus :: Bonus -> Maybe Throw -> Throw -> Bonus
        newBonus _ Nothing 10                   = (True,True)
        newBonus (b1,b2) l t | fmap (+t) l == Just 10 = (True,False)  
                             | otherwise              = (b2,False)
